...
function GameView:createScoreBoard()
    ...
    local logo = display.newSprite("#logo.png"):pos(100,topY):addTo(self)
...
function GameView:changeCellNum(sprite,num)
    if num < 4 then
        ...
        if label then
            sprite:removeSelf(label)
        end
        sprite:setDisplayFrame(display.newSpriteFrame(tostring(num)..".png"))
    else
        sprite:setDisplayFrame(display.newSpriteFrame("2.png"))
...
function GameView:showDigital(sprite,num)
    local label = sprite:getChildByTag(10)
    if label then
        sprite:removeChild(label)
    end
    if num <= 8192 then
        label = display.newSprite("#"..tostring(num)..".png")
            :pos(64,64)
            :addTo(sprite,10,10)
    else
...
function GameView:getCellSprite(num)
    ...
    if num < 4 then
        ...
        sprite = display.newSprite("#2.png")
        self:showDigital(sprite,num)
    end
    return sprite
end
